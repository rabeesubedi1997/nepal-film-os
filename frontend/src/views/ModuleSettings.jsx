...
export default function ModuleSettings() {
  const { currentFilm, userRole, toggleModule } = useAuthStore();
  const addToast = useToastStore(s => s.addToast);

  const isProducer = userRole === 'Producer' || userRole === 'Super Admin';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {isProducer && (
          <span className="flex items-center gap-1.5 text-xs text-amber-500 bg-amber-500/10 px-3 py-1.5 rounded-full font-medium">
            <Shield className="h-3.5 w-3.5" /> Producer Controls
          </span>
        )}
      </div>
      ...
      {!isProducer && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 text-center">
          ...
          <p className="text-slate-400 text-sm">Only Producers and Super Admins can manage feature settings.</p>
        </div>
      )}
