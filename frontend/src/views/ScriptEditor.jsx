...
export default function ScriptEditor() {
  ...
  const handlePrintPdf = () => {
    ...
    if (!w) { addToast('Popup blocked. Please allow popups for this site or use Ctrl+P to print.', 'error'); return; }
    const tpHtml = titlePageData ? buildTitlePageHtml(titlePageData) : '';
                ...
                {!showSource ? (
                  <div className={`flex-1 flex ${showPreview ? 'flex-row' : ''}`}>
                    <div className={`${showPreview ? 'w-1/2' : 'w-full'} flex flex-col min-w-0`}>
                      <div className="flex-1 overflow-y-auto min-h-0 screenplay-editor-wrapper relative" ref={editorContainerRef}>
